...
- **A hierarchy that leaves the walk says so** — `implements` resolves a chain by matching one walked type's base against another walked type's name, so a link declared outside the given paths ends the chain and takes every subtype below it with it. A transitive answer therefore reports `hierarchy_truncated` and `unseen_bases` (each `{name, path}`) in `ast-bro.implements.v1`, plus a stderr note naming those bases and the files that declare them. A base is listed only when the project is found to declare it outside the walk *and* its own declarations prove it inherits the target, so library types the walk could never resolve raise nothing, and neither does the target's own declaration (`implements` never needs it). Declarations are located by file name, ignoring case and word separators, so a type whose file is not named after it cannot be placed: those names come back in `unlocated_bases`, and an empty answer says on stderr that the search could not place them. Read that list in one direction only — **empty** means every name was accounted for, so `hierarchy_truncated: false` is exact; **non-empty** is the normal state of a Java or C# answer (it is the library surface the walked files inherit) and says nothing about whether this particular answer is short. `--direct` is never qualified.
